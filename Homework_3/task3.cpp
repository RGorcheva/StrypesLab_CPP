...
int count_words(const char* string, const char* del) 
{
	if (!string || !del)
	{
		return 0;
...
char** string_split(const char* string, const char* delimiter, bool keep_empty_elements)
{
	...
	int count_word = count_words(string, delimiter);
	char** separate_arr = (char**)malloc(sizeof(char*) * count_word);
	if (!separate_arr)
	{
		std::cerr << "In function " << func_name << ": Cannot allocate memory!" << std::endl;
		return nullptr;
...
int main(void)
{
	...
	int words = count_words(input, delimiter);
